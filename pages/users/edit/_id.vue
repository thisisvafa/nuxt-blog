...
export default {
  ...
  async fetch(){
    ...
    .then(response => {
      ...
      this.password_confirmation = response.data.user.password_confirmation
  ...
  methods: {
    ...
    async updateUser(){
      this.errors = []
      await this.$axios.put('/api/user/update/'+this.$route.params.id , {
        name: this.name,
  ...
  head(){
    return{
      title: 'Update User - ' + this.username
    }
  },
}
</script>
